...
from fairseq.sequance_scorer_decoder_position_probe import SequenceScorerDecoderPositionProbe as SequenceScorer
...
def eval_lm(
    ...
    for sample in batch_iterator:
        ...
        gen_timer.start()
        hypos = scorer.generate(models, sample, target_dictionary)
        ...
        for i, hypos_i in enumerate(hypos):
            ...
            dim = tokens.shape[0]
            positions_i = positions[:dim]
            tgt = torch.tensor(positions_i).expand(1, dim).to(sample['target'])
            n_correct += 1 if torch.all(torch.eq(tgt, hypo['hypo_tokens'])) else 0
            #TODO:Adi - add token level accuracy + edit distance
            if not torch.all(torch.eq(tgt, hypo['hypo_tokens'])):
                logger.info("hypo: {}\ninput: {}".format(target_dictionary.string(hypo['hypo_tokens']),
                            target_dictionary.string(sample["net_input"]['src_tokens'][i])))
...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    results = eval_lm(
        ...
        output_word_probs=cfg.eval_lm.output_word_probs, #TODO: set to true
        output_word_stats=cfg.eval_lm.output_word_stats, #TODO: set to true
    ...
    logger.info(
        "Loss (base 2): {:.4f}, Perplexity: {:.2f}, Accuracy: {}, Total: {}".format(
            results["loss"], results["perplexity"], results["acc"], results["total"]
        )
    )
